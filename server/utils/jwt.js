...
const USER_PICK = ['_id', 'userId', 'email', 'displayName', 'provider'];

const signJwt = (userData, userFromDB) => 
  new Promise(async (resolve, reject) => {
    const { _id, userId, provider, token } = userData;
    ...
    const payload = { token, exp: expiresIn, ...pick(_user, USER_PICK) };
    
    jwt.sign(
        payload, 
        Buffer.from(JWT_SECRET, 'base64'), 
        { issuer: _user.displayName, algorithm: JWT_ALGORITHM }, 
